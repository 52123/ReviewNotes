...
产生原因：基于计时器的重传往往要等待很长时间

如果连续三次收到重复的 ACK，就会重传对应包，而不需要等到计时器超时
...
产生原因：以上方案都不能解决重传需要传多少个包的问题



简单来讲就是在快速重传的基础上，**返回最近收到的报文段的序列号范围**，在 TCP Header 中增加 SACK 的选项，用于接收端标记已经收到的数据包**区间** 



##### 4.3.4 D-SACK  重复收到数据的问题
...
- RTT：往返时延，就是数据报从发出去到收到对应ACK的时间，它是针对链接的，每个连接都有独立的RTT
- RTO（Retransmission Time Out）：重传超时，TCP 在发送数据时会设置一个计数器，若计时器超时仍未收到数据确认信息，则会引发相应的超时或基于计时器的重传操作，计时器超时被称为重传超时
...
#### 2.1 快速地启动一个tcp server监听某个端口

> nc -l    快速地启动一个tcp server监听某个端口
...
-p 可以显示连接归属的进程信息，在查看端口被那个进程占用时非常有用
...
- 统计处于各个状态的连接个数
  ...
  > netstat -ant | awk '{print $6}' | sort | uniq -c | sort -n 
   ...
   使用akw截取出状态行，然后用sort、uniq进行去重和计数
...
- 过滤主机
  ...
  > tcpdum -i any host 10.211.55.10

- 过滤源地址、目的地址：src、dst

  > tcpdump -i any src 10.10.22.12  只抓取10.10.22.12发出的包
  >
  > tcpdump -i any dst 10.10.22.12

- 过滤端口：port

  > tcpdump -i any port 80
  >
  > tcpdump -i any dst port 80 只想抓端口80发出的包

- 过滤指定端口范围内的流量

  > tcpdump -i any portrange 21023

- 禁用主机与端口解析：-n与-nn选项

- 过滤协议

  > tcpdump -i any -nn udp 只想查看udp协议
...
上面的过滤器都可以用布尔运算符组合出任意复杂的过滤器



还可以根据TCP头部中偏移量来过滤指定类型的包

```
tcpdump 'tcp[13] & 18 != 0'  过滤SYN+ACK包
```
...
![image-20210320222625917](/Users/user/IdeaProjects/ReviewNotes/docs/TLS handshake.png)
...
![HTTPS客户端确认域名](/Users/user/IdeaProjects/ReviewNotes/docs/HTTPS客户端确认域名.png)
...
![man-in-middler_attack](/Users/user/IdeaProjects/ReviewNotes/docs/man-in-middler_attack.jpeg)
...
![证书链](/Users/user/IdeaProjects/ReviewNotes/docs/证书链.png)
...
![密钥协商过程时序图](/Users/user/IdeaProjects/ReviewNotes/docs/密钥协商过程时序图.png)
...
- 请求行：方法字段、URL字段和HTTP协议版本字段3个字段组成
- 请求头部：请求头部是对请求数据做描述的
- 空行：分割请求头部与请求数据
- 请求数据：
...
因为 HTTP 协议有一个缺陷：通信只能由客户端发起。这种单向请求的特点，注定了如果服务器有连续的状态变化，客户端要获知就非常麻烦
