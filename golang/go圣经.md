...
- 方法声明的集合
- 任何类型的对象实现了interface接口中声明的全部方法，则表明该类型实现了该接口
...
o的runtime和工具链为我们装备了一个复杂但好用的动态分析工具，竞争检查器（the race detector）
...
reflect.Value的CanAddr方法来判断其是否可以被取地址。每当我们通过指针间接地获取的reflect.Value都是可取地址的，即使开始的是一个不可取地址的Value





从变量对应的可取地址的reflect.Value来访问变量需要三个步骤。第一步是调用Addr()方法，它返回一个Value，里面保存了指向变量的指针。然后是在Value上调用Interface()方法，也就是返回一个interface{}，里面包含指向变量的指针。最后，如果我们知道变量的类型，我们可以使用类型的断言机制将得到的interface{}类型的接口强制转为普通的类型指针。这样我们就可以通过这个普通指针来更新变量了：
...
unsafe包是一个采用特殊方式实现的包。虽然它可以和普通包一样的导入和使用，但它实际上是由编译器实现的。它提供了一些访问语言内部特性的方法，特别是内存布局相关的细节。将这些特性封装到一个独立的包中，是为在极少数情况下需要使用的时候，同时引起人们的注意





## 11.1 unsafe.Sizeof，Alignof和Offsetof



### 11.1.1 unsafe.Sizeof
...
重要的意义是指针转换语法让我们可以在不破坏类型系统的前提下向内存写入任意的值
...
![image-20210908195858682](..\docs\GPM模型.png)
...
- M在绑定有效的P后，进入调度循环，从Global队列、P的Local队列以及wait队列中获取G，而且M并不保留G的状态，这是G可以跨M调度的基础
...
- P的最大作用是拥有各种G的对象队列、链表、cache和状态
- P的数量由用户设置的GOMAXPROCS决定，但是不论GOMAXPROCS设置为多大，P的数量最大为256



Go 1.0发布的时候，它的调度器其实G-M模型，也就是没有P的，调度过程全由G和M完成，这个模型暴露出一些问题：
...
新设计和实现了Go调度器（在原有的G-M模型中引入了P）并且实现了一个叫做 *work-stealing* 的调度算法：
...
- 在G的眼中只有P，P就是运行G的”CPU“。G并非执行体，每个G需要绑定到P才能被调度执行
- 相当于两级线程
...
用户线程与内核线程KSE是多对一（N : 1）的映射模型，一个进程中所有创建的线程都只和同一个KSE在运行时动态绑定
...
- 轻量级。线程调度实在用户态完成的，不需要上下文切换
- 资源消耗会小很多



缺点：**不能做到真正意义上的并发**，假如某个线程因为阻塞操作被CPU给中断了，那么整个进程都会被挂起，因为在用户级线程模型下，一个CPU关联运行的是整个用户进程，而用户进程的线程对CPU是不可见的（调度没有CPU中断），所以无法做到自调度

改正：很多协程库会**把自己一些阻塞操作重新封装为完全的非阻塞形式**，在要到阻塞的点上，主动让出自己，并通过某种方式通知活唤醒其他待执行的用户线程在该KSE上运行，从而避免了内核调度器由于KSE阻塞而做上下文切换
...
例子：Java的Thread、C++的std::thread

优点：实现简单，直接借助操作系统内核的线程以及调度器

缺点：小号资源较大。直接借助了操作系统内核来创建、销毁线程，以及多个线程之间的上下文切换和调度
...
其次，又区别于内核级线程模型，它的进程里的所有线程并不与KSE一一绑定，而是可以动态绑定同一个KSE， 当某个KSE因为其绑定的线程的阻塞操作被内核调度出CPU时，其关联的进程中其余用户线程可以重新与其他KSE绑定运行
...
> 线程想运行任务就得获取P，从P的本地队列获取G，P队列为空时，M从其他P的本地队列**偷**一半放到自己P的本地队列，也会尝试从全局队列**拿**一批G放到P的本地队列
...
![image-20210909094938593](..\docs\go-func调度流程.png)

- 有两个存储G的队列，一个是局部调度器P的本地队列，一个是全局G队列。新创建的G会先保存在P的本地队列中，如果P的本地队列已经满了，就会保存在全局队列中
- G只能运行在M中，一个M必须持有一个P。M会从P的本地队列弹出一个可执行状态的G来执行，如果P的本地队列为空，就会向其它的MP组合中偷取一半可执行的G来执行
- 一个M调度G执行的过程是一个循环机制
- 当M执行某个G时，如果发生了syscall或则其余阻塞操作，M就会阻塞。如果当前有一些G在执行，runtime会把这个线程M从P中摘除(detach)，然后再创建一个新的操作系统线程（有空闲则复用）来服务这个P
- 当M系统调用结束时，这个M会尝试获取一个空闲的P执行，并把G放到这个P的本地队列。如果获取不到P，那么这个线程M会变成休眠状态，加入到休眠队列中，然后这个G会被放入到全局队列中
...
- 每次启动一个M，都会创建一个goroutine，就是G0
- G0仅用于负责调度的G
- G0不指向任何可执行的函数
- 每个M都会有一个自己的G0
- 在调度或系统调用时，M会切换到G0，来调度
- M0的G0会放在全局空间
...
P拥有G1，M1获取P后开始运行G1，G1使用go func创建了G2，为了局部性G2优先加入到P1的本地队列
...
G2本地队列满再创建G7，将本地队列中前面一半的G连同G7一起打乱顺序，然后放入本地队列中
...
假定G2唤醒了M2，M2绑定了P2，并运行G0，但P2的本地队列没有G，M2此时为自选线程（没有G但为运行状态的线程，不断寻找G）



### 12.8.5 被唤醒的M2从全局队列批量取G 

从全局队列取的个数：n = min( len(GQ) / GOMAXPROCS + 1, len(GQ/2))  待确认



### 12.8.6 偷取G的情况

全局队列已经没有G，那m就要执行work stealing：从其他有G的P那里偷取一半G过来，放到自己的P本地队列。



### 12.8.7 自旋线程的最大限制

自旋线程 + 执行线程 <= GOMAXPROCS，多的线程会放入休眠队列
...
- G1创建G2，并且G1进行阻塞系统调用
- M1系统调用中，M1与P1解绑，P1寻找休眠的M进行重新绑定，没有则新建
- P1与空闲的M2绑定，通过G0切换到G2并执行



当G1变成了非阻塞，M1会记录之前绑定的P1，当P1被抢占失败时，查看空闲P队列是否有，没有则假如全局P队列，M1就会变成休眠队列
...
- 暂停程序业务逻辑，找出不可达的对象和可达的对象
- 开始标记，找出它所有可达的对象，并做上标记
- 标记完了之后，开始清除未标记的对象
- 停止暂停，让程序继续跑，然后循环重复这个过程
...
- 需要STW，暂停用户线程
- 标记需要扫描整个heap？没有辅助手段例如card table、mod union table吗
- 会有内存碎片
...
- 将所有对象标记为白色，放入到白色集合中
- 每次GC回收开始，会从根节点开始遍历所有对象，把遍历到的对象从白色集合放入灰色集合中(本次遍历是一次遍历，非递归形式)
- 遍历灰色集合，将灰色对象引用的对象从白色集合放入灰色集合，把已经遍历过的灰色对象放入黑色集合
- 重复第三步，直到灰色中无任何对象
- 回收所有的白色标记表中的对象，也就是垃圾



### 13.2.1 没有STW的三色标记法会出现的问题

因为是并发标记，所以可能会出现错标，漏标的情况
...
强三色不变式：不存在黑色对象引用到白色对象的指针，即破坏了条件一
...
操作：对象A引用对象B，B对象要被标记成灰色

满足：强三色不变式，不存在黑节点直接引用白色节点



黑色对象的内存槽有两种位置, `栈`和`堆`. 栈空间的特点是容量小,但是要求相应速度快,因为函数调用弹出频繁使用, 所以“插入屏障”机制,在**栈空间的对象操作中不使用**. 而仅仅使用在堆空间对象的操作中



- 程序开始时将全部对象标记为白色，将所有对象放入白色集合中
- 遍历Root Set（非递归，只遍历一次），得到灰色节点
- 遍历灰色标记表，将可达的对象，从白色标记为灰色，遍历之后的灰色，标记为黑色
- 由于并发特性，此刻栈和堆上的黑色对象被外界添加了白色对象，堆上触发插入屏障机制，栈上不触发
  - 堆上：由于插入写屏障，堆上新插入的白色节点改为灰色
  - 栈上：全部节点扫描完成后，需要进行带有STW的三色标记
- 继续循环上述流程，直到没有灰色节点
- 清楚白色节点
...
- 程序开始时将全部对象标记为白色，将所有对象放入白色集合中
- 遍历Root Set（非递归，只遍历一次），得到灰色节点
- 灰色对象删除白色对象，触发删除写屏障，**白色对象会变成灰色对象**
- 遍历灰色标记表，将可达的对象从白色变成灰色，遍历之后的灰色，标记为黑色
- 循环上述操作
- 清除白色节点
...
编译器会做逃逸分析，当发现变量的作用域没有抛出函数范围，就可以在栈上分配，反之则必须分配在堆上
...
#### 13.3.2.1 []interface{}数据类型，通过[]赋值必定会出现逃逸

#### 13.3.2.2 map[string]interface{} 类型尝试通过[]赋值，必定会出现逃逸

#### 13.3.2.3 map[interface{}]interface{}尝试通过[]赋值，key和value都会逃逸

#### 13.3.2.4 `map[string][]string`数据类型，赋值会发生`[]string`发生逃逸。

#### 13.3.2.5 `[]*int`数据类型，赋值的右值会发生逃逸现象。

#### 13.3.2.6 `func(*int)`函数类型，进行函数赋值，会使传递的形参出现逃逸现象

#### 13.3.2.7 `func([]string)`: 函数类型，进行`[]string{"value"}`赋值，会使传递的参数出现逃逸现象

#### 13.3.2.8 `chan []string`数据类型，想当前channel中传输`[]string{"value"}`会发生逃逸现象



### 13.3.3 逃逸规则

- 逃逸分析实在编译器完成得，这是不用于JVM的运行时逃逸分析
...
我们可通过`go build -gcflags '-m -l'`命令来查看逃逸分析结果，其中-m 打印逃逸分析信息，-l禁止内联优化

为了看到更多细节，可以在语句中再添加一个`-m`参数
...
func main() {
    a := 666
    fmt.Println(a) // 因为fmt.Println的函数参数为interface类型，编译期不能确定其参数的具体类型，所以将其分配于堆上
}
```

#### 13.3.4.2 暴露给外部指针

变量在函数外部存在引用。这个很好理解，因为当函数执行完毕，对应的栈帧就被销毁，但是引用已经被返回到函数之外。如果这时外部从引用地址取值，虽然地址还在，但是这块内存已经被释放回收了，这就是非法内存，问题可就大了。所以，很明显，这种情况必须分配到堆上
...
在`make`方法中，没有直接指定大小，而是填入了变量`n`，这时Go逃逸分析也会将其分配到堆区去。可见，为了保证内存的绝对安全，Go的编译器可能会将一些变量不合时宜地分配到堆上，但是因为这些对象最终也会被垃圾收集器处理，所以也能接受
